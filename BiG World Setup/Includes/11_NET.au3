...
; ---------------------------------------------------------------------------------------------
	For $d = 1 To $g_CurrentPackages[0][0] ; loop through the elements of an array
		...
		Local $ReadSection = IniReadSection($g_MODIni, $g_CurrentPackages[$d][0])
		$Prefix[3] = _GetTra($ReadSection, 'T') & '-Add' ; adjust the language-addon
		For $p = 1 To 3
			If $g_Flags[0] = 0 Then ; the exit button was pressed
				Exit
			EndIf
			Local $URL = _IniRead($ReadSection, $Prefix[$p] & 'Down', 'Manual')
			Local $File = _IniRead($ReadSection, $Prefix[$p] & 'Save', '') ; filename
			Local $Text = FileRead($g_LogFile)
...
; ---------------------------------------------------------------------------------------------
	...
	For $d = 1 To $DpS[0][0]
		...
		Local $Split = StringSplit(StringRegExpReplace($DpS[$d][1], '\A\x7c|\x7c\z', ''), '|')
		For $s = 1 To $Split[0]
	...
	AdlibRegister('_NET_Update_Progress', 1000) ; do progress updates with adlib so GUI updates on a regular basis
	Local $DSlot[6] = [5]
...
; ---------------------------------------------------------------------------------------------
		If $g_Flags[23] <> '' Then
			...
			Local $List = ProcessList('wget.exe') ; check wget-process only so we don't kill other processes
			For $l = 1 To $List[0][0]
				If $DArray[$DSlot[$d]][7] = $List[$l][1] Then
					...
					Local $Result = StdoutRead($DArray[$DSlot[$d]][7]) ; read the output of wget...
		...
		For $d = 1 To 5
			If $DSlot[$d] = '' Then ; no DArray-entry assigned to that slot
				...
				If $DpS[0][1] = '|1|' Then ; only shs-queue left. bws may use more than one download
					Local $Queue = 1
				Else
					Local $Available = $DpS[0][1] ; get current unused queues
					For $s = 1 To 5
				...
				ElseIf $Result = 0 Then ; fault
					...
					;EndIf
					$DpS[$DArray[$DSlot[$d]][6]][1] = StringReplace($DpS[$DArray[$DSlot[$d]][6]][1], '|' & $DSlot[$d] & '|', '|') ; remove from queue
					Local $Error = IniRead($g_BWSIni, 'Faults', $DArray[$DSlot[$d]][2], '')
		...
		If $Loop = 6 Then
			...
			For $d = 1 To $DSlot[0]
				...
				If TimerDiff($DArray[$DSlot[$d]][10]) > 20000 Then ; 20 seconds have passed since last test
					$DArray[$DSlot[$d]][10] = TimerInit() ; reset timer
					Local $localSize = FileGetSize($g_DownDir & '\' & $g_Down[$d][0])
...
Func Au3NetFix($p_Num = 0)
	...
	Local $ExtractOnlyMods = IniRead($g_GConfDir & '\Game.ini', 'Options', 'ExtractOnly', 'BWFixpack,BWTextpack,BWInstallpack,WeiDU,BeregostCF')
	Local $DownloadOnlyMods = IniRead($g_GConfDir & '\Game.ini', 'Options', 'DownloadOnly', 'BG1TP,BWPDF')
...
; ---------------------------------------------------------------------------------------------
	For $c = 1 To $g_CurrentPackages[0][0]
		Local $ReadSection = IniReadSection($g_MODIni, $g_CurrentPackages[$c][0])
		$Prefix[3] = _GetTra($ReadSection, 'T') & '-Add' ; adjust the language-addon
		For $p = 1 To $Prefix[0]
			Local $File = _IniRead($ReadSection, $Prefix[$p] & 'Down', 'Manual')
			...
			Local $localSize = FileGetSize($g_DownDir & '\' & $File)
			If $ExpectedSize <> $localSize Then
				Local $Error = IniRead($g_BWSIni, 'Faults', $g_CurrentPackages[$c][0], '')
	...
	If $g_pQuestion = 'p' Then
		...
		For $f = 1 To $Fault[0][0]
			...
			For $l = 1 To StringLen($Fault[$f][1])
				...
				Local $Save = _IniRead($ReadSection, $Prefix[$Type] & 'Save', '')
				$ExpectedSize = _IniRead($ReadSection, $Prefix[$Type] & 'Size', '')
				Local $Name = _IniRead($ReadSection, 'Name', $Fault[$f][0])
...
Func _Net_DownloadStart($p_URL, $p_File, $p_Setup, $p_Prefix, $p_String) ; Link, SaveAs, Setup-name, Prefix, Modname, Tested
	...
	Local $NetInfo = _Net_LinkUpdateInfo($p_URL, $p_File, $p_Setup, $p_Prefix)
	Local $Changed = @extended
	...
	Else ; file found
		...
		EndIf
		If FileExists($g_DownDir & '\' & $p_File) Then
			Local $localSize = FileGetSize($g_DownDir & '\' & $p_File)
...
Func _Net_FixSHSAttachment($p_Mod, $p_Prefix = '')
	...
	Local $Raw = StringTrimLeft(FileRead($g_DownDir & '\' & $Save), 1) ; read the file and strip the linefeed at the start
	Local $Handle = FileOpen($g_DownDir & '\' & $Save, 2)
	FileWrite($Handle, $Raw)
	FileClose($Handle)
	If Asc(FileRead($g_DownDir & '\' & $Save, 1)) <> 10 Then ; success
		FileDelete(@TempDir & '\' & $Save) ; remove backup
		Local $Size = FileGetSize($g_DownDir & '\' & $Save)
...
Func _Net_LinkListUpdate($p_List)
	...
	Local $Format = GUICtrlRead($g_UI_Interact[6][5])
	GUICtrlSetData($g_UI_Interact[6][2], '')
	Local $Handle = FileOpen($g_LogFile, 2)
	For $l = 1 To $p_List[0][0] ; loop through the list
		If $g_Flags[0] = 0 Or $g_Flags[11] = 1 Or $g_Flags[12] = 1 Then ExitLoop
		If $p_List[$l][0] = $p_List[$l - 1][0] Then ContinueLoop ; don't show links twice
		Local $ReadSection = IniReadSection($g_ModIni, $p_List[$l][0])
		Local $Mod = _IniRead($ReadSection, 'Name', $p_List[$l][0])
		$Prefix[3] = _GetTra($ReadSection, 'T') & '-Add' ; adjust the language-addon
		For $p = 1 To 3
			...
			If $Down <> 'MANUAL' Then
				Local $Output = StringReplace(StringReplace(StringReplace($Format, '%URL%', $Down), '%MOD%', $Mod), '|', @CRLF)
...
; ---------------------------------------------------------------------------------------------
	If $Return[2] = 0 Then
		...
		Local $hDownload = InetGet($p_URL, @TempDir & '\' & $Return[1], $INET_FORCERELOAD, $INET_DOWNLOADBACKGROUND)

		Do
			Sleep(50)
		Until InetGetInfo($hDownload, $INET_DOWNLOADSIZE(1000))
...
Func _Net_LinkUpdateInfo($p_URL, $p_File, $p_Setup, $p_Prefix)
	...
	Local $Return = _Net_LinkGetInfo($p_URL)
	If $Return[0] = 0 Then
		FileWrite($g_LogFile, '= NA' & @CRLF)
...
Func _Net_ListMissing()
	...
	For $f = 1 To $Fault[0][0]
		...
		For $l = 1 To StringLen($Fault[$f][1])
			Local $Type = StringMid($Fault[$f][1], $l, 1)
			If $Type = '1' Then
				Local $Hint = _GetTR($Message, 'L2') ; => main
			ElseIf $Type = '2' Then
...
Func _Net_LoginFastShare($p_URL)
	...
	Local $Handle = __HTTPConnect("fastshare.org")
...
Func _Net_LinkTest($p_Num = 0)
	...
	Local $Prefix[4] = [3, '', 'Add', $g_ATrans[$g_ATNum] & '-Add'], $TestedBefore = 0, $Fault
	Local $NeedInteract = StringTrimLeft(IniRead($g_TRAIni, 'Net-ListMissing', 'L6', ''), 3)
	$g_Flags[0] = 1
	_Process_Gui_Create(1, 0)
	Local $List = _Tree_GetCurrentList()
	GUICtrlSetData($g_UI_Interact[6][4], _GetSTR($Message, 'H1')) ; => help text
	...
	For $l = 1 To $List[0][0] ; loop through the list
		...
		For $p = 1 To 3
			...
			Else
				...
				Local $Size = Round($NetInfo[2] / 1048576, 2)
...
Func _Net_RemoveFixedFaults()
	...
	For $f = 1 To $Fault[0][0]
		...
		For $p = 1 To $Prefix[0]
			...
			Local $Save = _IniRead($ReadSection, $Prefix[$p] & 'Save', '')
			Local $Size = _IniRead($ReadSection, $Prefix[$p] & 'Size', '')
...
Func _Net_WGetShow($p_PID, $p_Num)
	...
	Local $Text = StdoutRead($p_PID, True)
	Local $OldNum = StringLen($Text)
	...
	While 1
		...
		If $Num <> $OldNum Then
			Local $NewText = StringRegExpReplace(_StringVerifyExtAscII(StringMid($Text, $OldNum + 1)), '\r\n|\x7c|\r\|\n', @CRLF)
			$Line = @extended
		...
		If $g_Flags[13] = 1 Or ProcessExists($p_PID) = 0 Then
			...
			ExitLoop
		EndIf
		Local $DoUpdate = StringRegExp(@OSVersion, 'WIN_VISTA|WIN_7|WIN_2008|WIN_2008R2')
		If $DoUpdate Then FileRead($g_DownDir & '\' & $g_Down[$p_Num][0], 1) ; files are not updated on windows 7. Use this as a workaround.
		Local $localSize = FileGetSize($g_DownDir & '\' & $g_Down[$p_Num][0])
...
Func _Net_WGetSize($p_URL)
	...
	Local $PID = Run('"' & $g_ProgDir & '\Tools\wget.exe" --no-passive-ftp --connect-timeout=20 --tries=1 ' & $param & ' --spider "' & $p_URL & '"', $g_ProgDir & '\Tools', @SW_HIDE, 8)
	Local $Success = ProcessWaitClose($PID, 20) ; wait for 20 seconds -- some servers are slow, e.g. eros.gram.pl
	...
	If StringLeft($p_URL, 3) = 'ftp' Then ; handle ftp-requests
		...
		Local $Size = StringRegExp(StringStripCR($Allines), '\n==> SIZE.*\n', 3)
	...
	Else
		...
		If StringInStr($Allines, 'Content-disposition:') Then
			$Return[0] = 1
			Local $Tmp = StringRegExp($Allines, "(?i)Content-disposition: (.*?)" & @CRLF, 3)
...
Func _Net_SingleLinkUpdate($p_Setup, $p_Update = 0)
	...
	Return ; new string
	Local $OldTime = StringTrimRight(FileGetTime($UpdateIni, 1, 1), 4)
	Local $NewTime = @YEAR & @MON & @MDAY & @HOUR
	...
	Local $ReadSection = IniReadSection($g_MODIni, $p_Setup)
	Local $New = IniReadSection($UpdateIni, $p_Setup)
	If @error Then Return '' ; this setup has no infos -> return
	For $n = 1 To $New[0][0]
		Local $OldValue = _IniRead($ReadSection, $New[$n][0], 'Manual') ; get the old entry
...
Func _Net_StartupUpdate()
	...
	If Not StringRegExp($g_Flags[14], 'BWP|BWS') Then ; currently no updates for other games than BWP
		...
		Return
	EndIf
	Local $Answer = _Misc_MsgGUI(2, _GetTR($Message, 'L1'), _GetTR($Message, 'L2'), 2, _GetTR($g_UI_Message, '0-B1'), _GetTR($g_UI_Message, '0-B2')) ; => look for configuration-updates now?
	...
	While 1
		Local $Ping = Ping('194.25.0.60', 1000) ; test if computer is online -- ip is a tcom-dns-server
	...
	Local $Success = _Net_Update_Link(2)
	If $Success = 0 Then
...
Func _Net_Update_Link($p_Show = 0) ; Show GUI
	...
	Local $UpdateIni = $g_ProgDir & '\Update\Mod.ini'
	Local $UpdateURL = IniRead($g_MODIni, 'BWS-URLUpdate', 'Down', 'http://baldurs-gate.eu/bws/mod.ini.gz')
	Local $UpdateArchive = IniRead($g_MODIni, 'BWS-URLUpdate', 'Save', 'mod.ini.gz')
; ---------------------------------------------------------------------------------------------
...
; ---------------------------------------------------------------------------------------------
	If StringInStr($p_Show, '2') Then
		...
		Local $SectionNames = IniReadSectionNames($UpdateIni)
		For $s = 1 To $g_Setups[0][0]
			...
			Local $h = 0 ; "mod is changed-line" is shown
			Local $NewSection = IniReadSection($UpdateIni, $g_Setups[$s][0]) ; has to exist we searched for sections from the update-file
			Local $OldSection = IniReadSection($g_MODIni, $g_Setups[$s][0])
			...
			For $n = 1 To $NewSection[0][0]
				...
				EndIf
				Local $OldValue = _IniRead($OldSection, $NewSection[$n][0], '') ; get the old entry
